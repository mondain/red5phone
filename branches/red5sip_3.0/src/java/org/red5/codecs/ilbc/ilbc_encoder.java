...
public class ilbc_encoder {
	...
	void AbsQuantW(float in[], /* (i) vector to encode */
		...
		if (state_first != 0) {
			...
			ilbc_common.AllPoleFilter(in, in_idx, weightDenum, weightDenum_idx, this.ULP_inst.state_short_len
		...
		for (lsp_index = 0; lsp_index < ilbc_constants.LPC_FILTERORDER; lsp_index++) {
			...
			for (step_idx = 0, step = steps[step_idx]; step_idx < ilbc_constants.LSF_NUMBER_OF_STEPS;) {
				...
				if (((hlp5 * (olds[old])) <= 0.0f) || (omega >= 0.5)) {
					...
					/*
					 * increment omega until they are of different sign, and we know there is at least one root between
					 * omega and old_omega
					 */
					olds[old] = hlp5;
					omega += step;
	...
	void SimpleInterpolateLSF(float syntdenum[], /*
												 ...
												 */
			float weightdenum[], /*
								 * (o) the weighting filter denominator resulting from the unquantized interpolated lsf
								 */
			...
			int block) /* (i) the sub-block number */
	{
		...
		/*
		 * Prepare search over one more codebook section. This section is created by filtering the original buffer with
		 * a filter.
		 */
		...
		for (stage = 0; stage < nStages; stage++) {
			...
			/*
			 * Loop over augmented part in the first codebook section, full search. The vectors are interpolated.
			 */
			...
			if (lTarget == ilbc_constants.SUBL) {
				...
				b[0] = best_index;
				c[0] = gain;
				searchAugmentedCB(20, 39, stage, base_size - lTarget / 2, target, buf, ilbc_constants.LPC_FILTERORDER
			...
			if (stage == 0) {
				...
				for (j = 0; j < (range - 1); j++) {
					energy[(ppe + 1)] = energy[ppe] + (cbvectors[ppi]) * (cbvectors[ppi]) - (cbvectors[ppo])
							* (cbvectors[ppo]);
					ppo--;
					ppi--;
					ppe++;
			...
			if ((lTarget == ilbc_constants.SUBL) && (sIndAug != 0)) {
				...
				b[0] = best_index;
				c[0] = gain;
				searchAugmentedCB(sIndAug, eIndAug, stage, 2 * base_size - 20, target, cbvectors, lMem, a, b, c,
						energy, invenergy);
				max_measure = a[0];
				best_index = b[0];
				gain = c[0];
			...
			if (stage == 0) {
				...
				if (stage == 1) {
					gain = ilbc_common.gainquant(gain, (float) (float) Math.abs(gains[stage - 1]), 16, gain_index,
							gain_index_idx + stage);
				} else {
					gain = ilbc_common.gainquant(gain, (float) (float) Math.abs(gains[stage - 1]), 8, gain_index,
							gain_index_idx + stage);
				}
			}
	...
	float residual[]) /* (i) lpc residual signal */
	{
		...
		float[] sampEn_win = { (float) 1.0f / (float) 6.0, (float) 2.0f / (float) 6.0, (float) 3.0f / (float) 6.0,
				(float) 4.0f / (float) 6.0, (float) 5.0f / (float) 6.0 };
			...
			float block[]) /* (o) speech vector to encode */
	{
		...
		ilbc_common.StateConstructW(idxForMax[0], idxVec, syntdenum,
				(start - 1) * (ilbc_constants.LPC_FILTERORDER + 1), decresidual, start_pos,
		...
		if (state_first != 0) { /* put adaptive part in the end */
			...
			iCBSearch(extra_cb_index, 0, extra_gain_index, 0, reverseResidual, 0, mem, ilbc_constants.CB_MEML
					- ilbc_constants.stMemLTbl, ilbc_constants.stMemLTbl, diff, ilbc_constants.CB_NSTAGES, weightdenum,
					(start - 1) * (ilbc_constants.LPC_FILTERORDER + 1), weightState, 0);
		...
		if (Nfor > 0) {
			...
			System.arraycopy(decresidual, (start - 1) * ilbc_constants.SUBL, mem, ilbc_constants.CB_MEML
					- ilbc_constants.STATE_LEN, ilbc_constants.STATE_LEN);
			...
			for (subframe = 0; subframe < Nfor; subframe++) {
				...
				iCBSearch(cb_index, subcount * ilbc_constants.CB_NSTAGES, gain_index, subcount
						* ilbc_constants.CB_NSTAGES, residual, (start + 1 + subframe) * ilbc_constants.SUBL, mem,
						ilbc_constants.CB_MEML - ilbc_constants.memLfTbl[subcount], ilbc_constants.memLfTbl[subcount],
						ilbc_constants.SUBL, ilbc_constants.CB_NSTAGES, weightdenum, (start + 1 + subframe)
								* (ilbc_constants.LPC_FILTERORDER + 1), weightState, subcount + 1);

				/* construct decoded vector */

				ilbc_common.iCBConstruct(decresidual, (start + 1 + subframe) * ilbc_constants.SUBL, cb_index, subcount
						* ilbc_constants.CB_NSTAGES, gain_index, subcount * ilbc_constants.CB_NSTAGES, mem,
						ilbc_constants.CB_MEML - ilbc_constants.memLfTbl[subcount], ilbc_constants.memLfTbl[subcount],
				...
				System.arraycopy(decresidual, (start + 1 + subframe) * ilbc_constants.SUBL, mem, ilbc_constants.CB_MEML
		...
		if (Nback > 0) {
			...
			for (n = 0; n < Nback; n++) {
				for (k = 0; k < ilbc_constants.SUBL; k++) {
					reverseResidual[n * ilbc_constants.SUBL + k] = residual[(start - 1) * ilbc_constants.SUBL - 1 - n
			...
			for (subframe = 0; subframe < Nback; subframe++) {
				...
				iCBSearch(cb_index, subcount * ilbc_constants.CB_NSTAGES, gain_index, subcount
						* ilbc_constants.CB_NSTAGES, reverseResidual, subframe * ilbc_constants.SUBL, mem,
						ilbc_constants.CB_MEML - ilbc_constants.memLfTbl[subcount], ilbc_constants.memLfTbl[subcount],
						ilbc_constants.SUBL, ilbc_constants.CB_NSTAGES, weightdenum, (start - 2 - subframe)
								* (ilbc_constants.LPC_FILTERORDER + 1), weightState, subcount + 1);

				/* construct decoded vector */

				ilbc_common.iCBConstruct(reverseDecresidual, subframe * ilbc_constants.SUBL, cb_index, subcount
						* ilbc_constants.CB_NSTAGES, gain_index, subcount * ilbc_constants.CB_NSTAGES, mem,
						ilbc_constants.CB_MEML - ilbc_constants.memLfTbl[subcount], ilbc_constants.memLfTbl[subcount],
						ilbc_constants.SUBL, ilbc_constants.CB_NSTAGES);
				...
				// memcpy(mem, mem+ilbc_constants.SUBL,
				// (ilbc_constants.CB_MEML-ilbc_constants.SUBL)*sizeof(float));
				System.arraycopy(reverseDecresidual, subframe * ilbc_constants.SUBL, mem, ilbc_constants.CB_MEML
						- ilbc_constants.SUBL, ilbc_constants.SUBL);
				// memcpy(mem+ilbc_constants.CB_MEML-ilbc_constants.SUBL,
