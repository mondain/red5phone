...
class ilbc_common {
	...
	public static void AllZeroFilter(float In[], /*
			...
			int orderCoef, /* (i) number of filter coefficients */
			float Out[], /*
						 * (i/o) on entrance Out[-orderCoef] to Out[-1] contain the filter state, on exit Out[0] to
	...
	public static void ZeroPoleFilter(float In[], /*
			...
			int In_idx, float ZeroCoef[],/*
										 * (i) filter coefficients for all-zero section (ZeroCoef[0] is assumed to be
										 * 1.0f)
										 */
			float PoleCoef[],/*
							 * (i) filter coefficients for all-pole section (ZeroCoef[0] is assumed to be 1.0f)
							 */
			...
			int orderCoef, /* (i) number of filter coefficients */
			float Out[], /*
						 * (i/o) on entrance Out[-orderCoef] to Out[-1] contain state of all-pole section. On exit
						 * Out[0] to Out[lengthInOut-1] contain filtered samples
						 */
			int Out_idx) {
	...
	public static void lsf2a(float a_coef[], float freq[]) {
		...
		/*
		 * Check input for ill-conditioned cases. This part is not found in the TIA standard. It involves the following
		 * 2 IF blocks. If "freq" is judged ill-conditioned, then we first modify freq[0] and freq[LPC_HALFORDER-1]
		 * (normally LPC_HALFORDER = 10 for LPC applications), then we adjust the other "freq" values slightly
		 */
		...
		/*
		 * p[i] and q[i] compute cos(2*pi*omega_{2j}) and cos(2*pi*omega_{2j-1} in eqs. 4.2.2.2-1 and 4.2.2.2-2. Note
		 * that for this code p[i] specifies the coefficients used in .Q_A(z) while q[i] specifies the coefficients used
		 * in .P_A(z)
		 */
