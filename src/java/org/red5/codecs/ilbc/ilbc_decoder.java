...
public class ilbc_decoder {
			...
			float in[]) /* (i) unenhanced signal */
	{
		...
		for (iblock = 0; iblock < ilbc_constants.ENH_NBLOCKS - ioffset; iblock++) {
			...
			lag = 10;
			maxcc = xCorrCoef(downsampled, 60 + iblock * ilbc_constants.ENH_BLOCKL_HALF, downsampled, 60 + iblock
					* ilbc_constants.ENH_BLOCKL_HALF - lag, ilbc_constants.ENH_BLOCKL_HALF);
			for (ilag = 11; ilag < 60; ilag++) {
				cc = xCorrCoef(downsampled, 60 + iblock * ilbc_constants.ENH_BLOCKL_HALF, downsampled, 60 + iblock
						* ilbc_constants.ENH_BLOCKL_HALF - ilag, ilbc_constants.ENH_BLOCKL_HALF);
		...
		if (this.prev_enh_pl == 1) {
			...
			/*
			 * compute new concealed residual for the old lookahead, mix the forward PLC with a backward PLC from the
			 * new frame
			 */
			...
			if (ftmp1 > (float) 2.0f * ftmp2 && ftmp1 > 0.0) {
				...
				for (i = plc_blockl - 10; i < plc_blockl; i++) {
					plc_pred[i] *= (float) (i - plc_blockl + 10) * ((float) 1.0f - (float) 2.0 * ftmp2 / ftmp1)
							/ (float) (10) + (float) 2.0f * ftmp2 / ftmp1;
				}
			}
		...
		if (this.ULP_inst.mode == 20) {
			...
			for (iblock = 0; iblock < 2; iblock++) {
				enhancer(out, iblock * ilbc_constants.ENH_BLOCKL, enh_buf, ilbc_constants.ENH_BUFL, (5 + iblock)
						* ilbc_constants.ENH_BLOCKL + 40, ilbc_constants.ENH_ALPHA0, enh_period,
						ilbc_constants.enh_plocsTbl, ilbc_constants.ENH_NBLOCKS_TOT);
			}
		} else if (this.ULP_inst.mode == 30) {
			/* Enhancer with 80 samples delay */
			for (iblock = 0; iblock < 3; iblock++) {
				enhancer(out, iblock * ilbc_constants.ENH_BLOCKL, enh_buf, ilbc_constants.ENH_BUFL, (4 + iblock)
						* ilbc_constants.ENH_BLOCKL, ilbc_constants.ENH_ALPHA0, enh_period,
						ilbc_constants.enh_plocsTbl, ilbc_constants.ENH_NBLOCKS_TOT);
			}
		}
			...
			int lpc_idx, int inlag) /* (i) pitch lag */
	{
		...
		if (PLI == 1) {
			...
			for (i = 0; i < this.ULP_inst.blockl; i++) {
				...
				if (i < 80)
					PLCresidual[i] = use_gain * (pitchfact * PLCresidual[i] + ((float) 1.0f - pitchfact) * randvec[i]);
				else if (i < 160)
					PLCresidual[i] = (float) 0.95 * use_gain
							* (pitchfact * PLCresidual[i] + ((float) 1.0f - pitchfact) * randvec[i]);
				else
	...
	public void Decode(float decresidual[], /* (o) decoded residual frame */
			...
			int extra_gain_index[], /*
									 ...
									 */
			int state_first) /*
							 * (i) 1 if non adaptive part of start state comes first 0 if that part comes last
							 */
	{
	...
	public void iLBC_decode(float decblock[], /* (o) decoded signal block */
					 ...
					 */
	{
		...
		if (mode > 0) { /* the data are good */
			...
			for (ulp = 0; ulp < 3; ulp++) {
				...
				for (i = 0; i < this.ULP_inst.nasub; i++) {
					for (k = 0; k < ilbc_constants.CB_NSTAGES; k++) {
						...
						cb_index[i * ilbc_constants.CB_NSTAGES + k] = bytes.packcombine(cb_index[i
								* ilbc_constants.CB_NSTAGES + k], lastpart, this.ULP_inst.cb_index[i][k][ulp]);
				...
				for (i = 0; i < this.ULP_inst.nasub; i++) {
					for (k = 0; k < ilbc_constants.CB_NSTAGES; k++) {
						...
						gain_index[i * ilbc_constants.CB_NSTAGES + k] = bytes.packcombine(gain_index[i
								* ilbc_constants.CB_NSTAGES + k], lastpart, this.ULP_inst.cb_gain[i][k][ulp]);
					}
				}
			}
			/*
			...
			if (mode == 1) { /*
				...
				doThePLC(PLCresidual, PLClpc, 0, decresidual, syntdenum, (ilbc_constants.LPC_FILTERORDER + 1)
		...
		if (this.use_enhancer == 1) {
			...
			if (this.ULP_inst.mode == 20) {
				...
				i = 0;
				syntFilter(data, i * ilbc_constants.SUBL, this.old_syntdenum, (i + this.ULP_inst.nsub - 1)
				...
				for (i = 1; i < this.ULP_inst.nsub; i++) {
					syntFilter(data, i * ilbc_constants.SUBL, syntdenum,
							(i - 1) * (ilbc_constants.LPC_FILTERORDER + 1), ilbc_constants.SUBL, this.syntMem);
				}

			} else if (this.ULP_inst.mode == 30) {
				/* Enhancer has 80 samples delay */
				for (i = 0; i < 2; i++) {
					syntFilter(data, i * ilbc_constants.SUBL, this.old_syntdenum, (i + this.ULP_inst.nsub - 2)
							* (ilbc_constants.LPC_FILTERORDER + 1), ilbc_constants.SUBL, this.syntMem);
				}
				for (i = 2; i < this.ULP_inst.nsub; i++) {
					syntFilter(data, i * ilbc_constants.SUBL, syntdenum,
							(i - 2) * (ilbc_constants.LPC_FILTERORDER + 1), ilbc_constants.SUBL, this.syntMem);
				}
			}
