...
public class SIPUserAgent extends CallListenerAdapter {
	...
	public void initSessionDescriptor() {
		...
		SessionDescriptor newSdp = SdpUtils.createInitialSdp(userProfile.username, sipProvider.getViaAddress(), userProfile.audioPort, userProfile.videoPort,
	...
	public SIPUserAgent(SipProvider sip_provider, SIPUserAgentProfile user_profile, SIPUserAgentListener listener, IMediaReceiver mediaReceiver) {
	...
	protected void launchMediaApplication() {
		...
		printLog("launchMediaApplication", "localAudioPort = " + localAudioPort + ", localVideoPort = " + localVideoPort + ".");
		...
		printLog("launchMediaApplication", "remoteAudioPort = " + remoteAudioPort + ", remoteVideoPort = " + remoteVideoPort + ".");
		...
		printLog("launchMediaApplication", "user_profile.audio = " + userProfile.audio + ", user_profile.video = " + userProfile.video + ", audio_app = " + audioApp
				+ ", video_app = " + videoApp + ".");
		...
		if (userProfile.audio && localAudioPort != 0 && remoteAudioPort != 0) {
			...
			if (audioApp == null) {
				...
				if (sipCodec != null) {
					...
					audioApp = new SIPAudioLauncher(sipCodec, localAudioPort, remoteMediaAddress, remoteAudioPort, mediaReceiver);
				} else {
		...
		if (userProfile.video && localVideoPort != 0 && remoteVideoPort != 0) {
			...
			if (videoApp == null) {
				...
				printLog("launchMediaApplication", "No external video application nor JMF has been provided: Video not started.");
				return;
			}
