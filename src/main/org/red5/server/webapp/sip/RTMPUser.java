...
//------------------------------------------------------------------------
	...
	public void pushAudio(int len, byte[] audio, long ts, int codec) throws IOException {
		...
		buffer.clear();
		buffer.put((byte)codec);	//first byte 2 mono 5500; 6 mono 11025; 22 mono 11025 adpcm 82 nellymoser 8000 178 speex 8000
		buffer.put(audio);
	...
	private class PlayNetStream extends AbstractClientStream implements IEventDispatcher {
		...
		public void dispatchEvent(IEvent event) {
			...
			if (rtmpEvent instanceof VideoData) {
				...
				kt2++; if (kt2 < 10) logger.debug("*** " + data.length);

				try {

					if (rtpStreamSender != null) rtpStreamSender.send(data, 1, data.length - 1);
				}
				catch(Exception e) {
