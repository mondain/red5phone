...
public class RTMPUser extends RTMPClient implements INetStreamEventHandler, IPendingServiceCallback {
...
//------------------------------------------------------------------------
	...
	public void startStream(String host, String app, int port, String publishName, String playName, PipedOutputStream rtmpOutputStream) {
		System.out.println("RTMPUser startStream");

		this.publishName = publishName;
		this.playName = playName;
		this.rtmpOutputStream = rtmpOutputStream;
		...
		try {
			...
			while (!startPublish) Thread.yield();
		}
		catch(Exception e) {
...
//------------------------------------------------------------------------
	...
	public void pushAudio(int len, byte[] audio, long ts, boolean compress) throws IOException {
		...
		if (compress) 				 //first byte 2 mono 5500; 6 mono 11025; 22 mono 11025 adpcm
			buffer.put((byte)22);
		else
			buffer.put((byte)6);
		...
		kt++; if (kt < 10) logger.debug("+++ " + audioData);
	...
	private class PlayNetStream extends AbstractClientStream implements IEventDispatcher {
		...
		public void dispatchEvent(IEvent event) {
			...
			if (logger.isDebugEnabled()) {
				//logger.debug("rtmp event: " + rtmpEvent.getHeader() + ", " + rtmpEvent.getClass().getSimpleName());
			}
			...
			if (rtmpEvent instanceof VideoData) {
				...
				kt2++; if (kt2 < 10) logger.debug("*** " + data);

				try {
					rtmpOutputStream.write(data, 1, data.length - 1);
					rtmpOutputStream.flush();
				}
				catch(Exception e) {
